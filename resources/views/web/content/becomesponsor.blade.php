...
@php
  ...
  // ========= افتراضات + كشف الدولة من الـ IP (torann/geoip) =========
  $defaultIso  = 'AE';
  ...
  try {
      ...
      // fallback للّافتراضيات
  }
@endphp

<style>
  .page-header__inner{ text-align:center; padding:60px 0; }
  #typed-title{
    display:inline-block; overflow:hidden; white-space:nowrap;
    border-right:3px solid #FFE986; font-family:'Montserrat',sans-serif;
    font-size:60px; background:linear-gradient(90deg,#FFE986 0%,#C48127 100%);
    -webkit-background-clip:text; -webkit-text-fill-color:transparent; margin-bottom:0;
  }
  @media (max-width:576px){ #typed-title{ font-size:40px; } .page-header__inner{ padding:80px 0 20px; } }

  body{ background:#f2f2f2; }
  .form-card{
    max-width:1000px; margin:50px auto;
    background:linear-gradient(90deg, rgb(60,42,30) 0%, rgb(26,18,11) 70%);
    border-radius:20px; padding:40px; box-shadow:0 8px 30px rgba(0,0,0,.08);
  }
  .form-title{
    font-size:30px; font-weight:800; text-align:center; margin-bottom:25px;
    background:linear-gradient(90deg,#FFE986 0%,#C48127 100%); -webkit-background-clip:text; -webkit-text-fill-color:transparent;
  }
  .form-wrap{ position:relative; margin-bottom:20px; }
  .form-input,.form-select{
    width:100%; padding:15px 18px; padding-left:45px; border:2px solid #ffe98685; border-radius:12px;
    font-size:16px; font-weight:500; transition:.3s; background:#211a1a; color:#fff;
  }
  .form-input::placeholder{ color:#aaa; }
  .form-input:focus,.form-select:focus{ border-color:#FFE986; box-shadow:0 0 0 3px rgba(231,55,1,.15); outline:none; }
  .form-icon{ position:absolute; top:50%; left:15px; transform:translateY(-50%); font-size:18px; color:#ffe98685; }
  .btn-square{
    width:100%; padding:14px; background:linear-gradient(90deg,#FFE986 0%,#C48127 100%);
    border:none; border-radius:50px; color:#fff; font-size:17px; font-weight:700; cursor:pointer;
    transition:transform .2s ease, opacity .3s ease;
  }
  .btn-square:hover{ transform:scale(1.03); opacity:.95; }
  .alert-danger{ font-size:14px; border-radius:8px; padding:12px; background:#ff3b3b; color:#fff; }
  .phone-flex{ display:flex; gap:10px; align-items:center; }

  /* ===== Scroll للقائمة: نحول الـ select إلى listbox مؤقتًا مع max-height + overflow ===== */
  .select-wrapper{ position:relative; width:260px; }
  .form-select.country{ padding-left:14px; }
  .form-select.country.expanded{
    position:absolute; left:0; right:0;
    z-index:30; max-height:280px; overflow-y:auto; /* Scroll */
    border-radius:12px;
  }
...
<section class="page-header">
  <div class="container">
    <div class="page-header__inner">
      <h2 id="typed-title">Become Sponsor</h2>
    </div>
  </div>
</section>
...
<section>
  <div class="container">
    <div class="form-card">
      ...
      <form id="reg-form" action="{{ route('web.register.store') }}" method="post" novalidate>
        @csrf
        ...
        <div class="form-wrap">
          ...
          <div class="phone-flex">
            <div class="select-wrapper">
              ...
              >
                @foreach($orderedIso as $iso)
                  ...
                  @endphp
                  <option value="{{ $dial }}" data-iso="{{ $iso }}" {{ $selected ? 'selected' : '' }}>
                    {{ $label }}
                  </option>
                @endforeach
              </select>
            ...
            <input class="form-input" id="phone" type="tel" name="phone" placeholder="Phone" inputmode="tel" required>
          </div>
...
(function(){
  ...
  function expand(){
    ...
    select.size = Math.min(10, select.options.length); // يعرض 10 عناصر مع Scroll
    // اختياري: نقل التركيز ليفتح على طول في بعض المتصفحات
    select.focus();
  }
  function collapse(){
    select.classList.remove('expanded');
    select.size = 0; // يرجعه Dropdown طبيعي (يساوي 1)
  }

  select.addEventListener('focus', expand);
  select.addEventListener('click', function(){
    // بعض المتصفحات لا تطبق focus مباشرة عند click
    if (!select.classList.contains('expanded')) expand();
  });
  ...
  select.addEventListener('blur', () => setTimeout(collapse, 100));

  // غلق عند الضغط خارج القائمة
  document.addEventListener('click', (e)=>{
    if (!select.contains(e.target)) collapse();
  });

  // ثبّت الافتراضي القادم من السيرفر
  const initialIso = (select.dataset.initialIso || '').toUpperCase();
  if (initialIso) {
    const opt = [...select.options].find(o => (o.dataset.iso||'').toUpperCase() === initialIso);
    if (opt) { select.value = opt.value; if (isoHidden) isoHidden.value = initialIso; }
  }
})();

/* ===================== اكتشاف البلد من "موقع الجهاز" (Geolocation) ===================== */
(function(){
  // خريطة ISO -> Dial مطابقة للـ PHP
  const isoToDial = @json($dialMap);
  const selectEl  = document.getElementById('country_code');
  const isoHidden = document.getElementById('country_iso');
  const phoneEl   = document.getElementById('phone');

  // صناديق حدود تقريبية (latMin, latMax, lonMin, lonMax)
  const countryBoxes = {
    ...
    ZA:[-35,-22,16,33], SN:[12,16.9,-17.7,-11], NG:[4,14.2,2.7,14.7], GH:[4.5,11.2,-3.3,1.2], KE:[-4.8,5.2,33.9,41.9], TZ:[-11.8,-0.8,29.3,40.5], UG:[-1.5,4.3,29.5,35], ET:[3.3,14.9,32.9,48.0],
  ...
  function setCountryByISO(iso){
    ...
    const desired = isoToDial[iso];
    const exists = [...selectEl.options].some(o => o.value === desired);
    if (exists) {
  ...
  // لو المستخدم بدأ يكتب رقم، ما نغيّرش اختياره
  let userTypedPhone = false;
  phoneEl.addEventListener('input', () => { if (phoneEl.value.trim().length > 0) userTypedPhone = true; });

  if ('geolocation' in navigator) {
    navigator.geolocation.getCurrentPosition(
      (pos)=>{
        if (userTypedPhone) return;
        const { latitude: lat, longitude: lon } = pos.coords || {};
        // أولوية: مصر ثم أي صندوق مطابق
        if (inBox(lat, lon, countryBoxes.EG)) { setCountryByISO('EG'); return; }
        for (const [iso, box] of Object.entries(countryBoxes)){
          if (inBox(lat, lon, box)) { setCountryByISO(iso); return; }
        }
      },
      (_err)=>{ /* تجاهل، هنفضل على الافتراضي */ },
      { enableHighAccuracy:false, timeout:3000, maximumAge:600000 }
    );
  }
})();

/* ===================== تحقّق الهاتف (libphonenumber + fallback) ===================== */
(function(){
  ...
  form.addEventListener('submit', function(e){
    ...
    try {
      ...
      if (parsed && parsed.isValid() && (!parsed.country || parsed.country === iso)) valid = true;
    } catch(_) {}

    if (!valid) {
      const fb = {
        'EG': /^0?(10|11|12|15)\d{8}$/, 'SA': /^0?5\d{8}$/, 'AE': /^0?5\d{8}$/, 'OM': /^0?9\d{7}$/,
        'US': /^\d{10}$/, 'GB': /^0?7\d{9}$/, 'IN': /^[6-9]\d{9}$/
      };
