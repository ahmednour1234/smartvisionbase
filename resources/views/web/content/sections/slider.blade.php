@php
    ...
    // ✅ date_active: default 1 لو مش موجود
    $dateActive = (int)($event->date_active ?? 1);

    // ✅ لو date_active = 0 => ممنوع نعمل parse للتاريخ
    $eventDate = null;
    $isFuture  = false;
    ...
    // ✅ عرض Coming Soon فقط لما date_active = 0 (بغض النظر عن التاريخ)
    $showComingSoon = ($dateActive === 0);

    // ✅ عرض countdown فقط لما date_active = 1 + تاريخ موجود + في المستقبل
    $showCountdown = ($dateActive === 1 && $eventDate && $isFuture);

    // Event fields
    $eventTitle    = $event->name_en ?? 'Event Title';
...
<section class="banner-one">
    ...
    <div class="container">
        <div class="banner-one__inner text-center px-3">
            ...
            @if($showCountdown)
                <div class="banner-one__countdown-timer-box"
                    ...
                    <div class="countdown time-countdown-two"
                         data-countdown-time="{{ $eventDate->format('Y/m/d H:i:s') }}">
                    </div>
                </div>
            ...
            {{-- ✅ التاريخ (يتعرض فقط لو date_active = 1 والتاريخ موجود) --}}
            @if($dateActive === 1 && $eventDate)
                <p class="banner-one__date"
                   data-aos="fade-up"
                   data-aos-delay="500"
                   style="font-family: 'Open Sans', sans-serif; font-size: 24px; color: #fff;">
                    {{ $eventDate->translatedFormat('F jS, Y') }}
                </p>
            @endif
...
{{-- Responsive + ComingSoon Style --}}
<style>
    .banner-one__comingsoon-box{
        display: inline-flex;
    ...
    @media (max-width: 768px) {
        .banner-one__title { font-size: 32px !important; }
        .banner-one__date,
        .banner-one__address,
        .banner-one__countdown-timer-box { font-size: 18px !important; }
        .banner-one__btn { font-size: 16px; padding: 10px 20px; }
        .banner-one__inner { padding-top: 40px; padding-bottom: 40px; }

        .banner-one__comingsoon-title{ font-size: 26px; }
        .banner-one__comingsoon-sub{ font-size: 13px; }
    }

    @media (max-width: 480px) {
        .banner-one__title { font-size: 28px !important; }
        .banner-one__btn { padding: 8px 16px; font-size: 14px; }
    }

    @media (max-width: 576px) {
      .banner-one__countdown-timer-box { font-size: 14px; }
    ...
    @media only screen and (max-width: 767px) {
        .banner-one { padding: 120px 0 220px; }
    }
</style>
