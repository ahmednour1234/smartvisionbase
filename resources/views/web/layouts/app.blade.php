@php
    ...
    $event   = \App\Models\Event::where('event_date', '>', now()->subCentury())->orderBy('event_date')->first(); // أمان
...
<head>
    ...
    <title>Middle East Financial Markets Awards Ceremony</title>

    <!-- favicons Icons -->
    <link rel="apple-touch-icon" sizes="180x180" href="{{ asset('public/'.$setting->img) }}?v={{ time() }}" />
    <link rel="icon" type="image/png" sizes="32x32" href="{{ asset('public/'.$setting->img) }}?v={{ time() }}" />
    <link rel="icon" type="image/png" sizes="16x16" href="{{ asset('public/'.$setting->img) }}?v={{ time() }}" />
    ...
    <style>
        ...
        /* حالة الانتهاء: كبّر الخط ووسّطه */
...
<body class="body-bg-color-1">
  ...
  {{-- ====== Countdown Bar ====== --}}
  <div class="countdown-section {{ $eventEnded ? 'ended' : '' }}" id="countdownSection">
    <div class="container countdown-container">
      <div class="countdown-text">
        ...
        @if($waClean)
          <a href="https://wa.me/{{ $waClean }}" target="_blank" class="whatsapp-icon" aria-label="WhatsApp">
            <i class="bi bi-whatsapp"></i>
          </a>
        @endif
      </div>

      {{-- الأزرار تظهر فقط لو الحدث لم ينتهِ --}}
      @unless($eventEnded)
  ...
  <script>
    (function(){
      ...
      const eventDateStr = @json($eventDate); // null لو مفيش
      ...
      // لو السيرفر قال انتهى أو مفيش تاريخ → اعرض النهاية فورًا
      if (endedServer || !eventDateStr) {
        setEnded();
      } else {
