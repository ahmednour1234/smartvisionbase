      ...
      <table class="table align-middle text-center">
        <thead>
          <tr>
            ...
            <th class="text-nowrap">{{ __('multi_media.actions') }}</th>
          </tr>
        </thead>
        <tbody>
          @forelse($multiMedias as $media)
            <tr>
              <td>{{ $loop->iteration + ($multiMedias->currentPage() - 1) * $multiMedias->perPage() }}</td>
              <td>{{ $media->name_ar }}</td>
              <td>{{ $media->name_en }}</td>
              <td>{{ $media->date }}</td>
              <td>
                <form action="{{ route('dashboard.multi-medias.activate', $media->id) }}" method="POST">
                  @csrf
                  <button type="submit" class="btn btn-sm btn-{{ $media->active ? 'success' : 'secondary' }}">
              ...
              <td class="text-nowrap">
                ...
                {{-- Destroy --}}
                <form action="{{ route('dashboard.multi-medias.destroy', $media->id) }}"
                      method="POST"
                      class="d-inline"
                      onsubmit="return confirm('{{ __('multi_media.confirm_delete') ?? __('Are you sure you want to delete this item?') }}');">
                  @csrf
                  @method('DELETE')
                  <button type="submit" class="btn btn-sm btn-danger">
                    <i class="bi bi-trash"></i> {{ __('multi_media.delete') }}
                  </button>
                </form>
              </td>
            </tr>
    ...
    @if ($multiMedias->hasPages())
      <div class="mt-4 d-flex justify-content-center">
        <nav class="inline-flex rounded-md shadow-sm" aria-label="Pagination">
          ...
          @else
            <a href="{{ $multiMedias->previousPageUrl() }}" class="px-3 py-2 text-sm bg-white text-gray-700 border border-gray-300 hover:bg-gray-100 rounded-start">‹</a>
          @endif
          ...
          @if ($multiMedias->hasMorePages())
            <a href="{{ $multiMedias->nextPageUrl() }}" class="px-3 py-2 text-sm bg-white text-gray-700 border border-gray-300 hover:bg-gray-100 rounded-end">›</a>
          @else
