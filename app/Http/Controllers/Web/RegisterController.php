...
$smsText .= "Phone: {$data['country_code']}{$data['phone']}\n";
        ...
        try {
            ...
            $this->sendClientQrWithFallback($client, $qrUrl, $qrImageUrl, $context);
            ...
            return back()->with('success', 'تم تسجيلك بنجاح، تم إرسال الدعوة على بريدك.');
            ...
            return back()->with('warning', 'تم التسجيل لكن فشل إرسال الإيميل. الرجاء التحقق من بريدك لاحقًا أو التواصل معنا.');
        }
    }
    ...
    protected function sendClientQrWithFallback(Client $client, string $qrUrl, string $qrImageUrl, array $context): void
    {
        ...
        try {
            $tls = array_merge($orig, [
                'port'       => 587,
                'encryption' => 'tls',
                // 'local_domain' => config('mail.mailers.smtp.local_domain', null), // اختياري
            ]);
            ...
            // رجّع الإعدادات الأصلية قبل أي محاولة أخرى
        ...
        try {
            ...
            throw $e; // ارمِ الاستثناء بعد فشل كل المحاولات
        } finally {
