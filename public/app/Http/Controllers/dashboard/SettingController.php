...
public function store(Request $request)
{
    ...
    if ($request->hasFile('img')) {
       ...
       if ($request->hasFile('img_mobile')) {
        // Get the image file
        $image = $request->file('img_mobile');

        // Generate a unique name for the image file to avoid overwriting
        $imageName = time() . '-' . $image->getClientOriginalName();

        // If there is an existing image, delete the old one
        if ($settings && $settings->img_mobile) {
            $oldImagePath = public_path($settings->img_mobile); // Path of the old image
            if (file_exists($oldImagePath)) {
                unlink($oldImagePath); // Delete the old image file
            }
        }
        ...
        // Add the image path to the validated data
        $validated['img_mobile'] = $imageName; // Store the image name directly (no subfolder)
    }
   if ($request->hasFile('floor_plan')) {
        ...
        // If there is an existing image, delete the old one
        if ($settings && $settings->floor_plan) {
            $oldImagePath = public_path($settings->floor_plan); // Path of the old image
            if (file_exists($oldImagePath)) {
                unlink($oldImagePath); // Delete the old image file
            }
        }
        ...
        // Add the image path to the validated data
        $validated['floor_plan'] = $imageName; // Store the image name directly (no subfolder)
    }
