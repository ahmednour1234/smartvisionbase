...
{
    // نتوقع إن الـ IDs جاية في مصفوفة اسمها ids[]
    $ids = $request->input('ids', []);

    if (empty($ids) || !is_array($ids)) {
        return redirect()
            ->route('admin.sponsors.index')
            ->with('error', __('general.no_items_selected'));
    }

    // حذف كل السبونسرز اللي IDs بتاعتهم في المصفوفة
    Sponsor::whereIn('id', $ids)->delete();

    return redirect()
        ->route('admin.sponsors.index')
        ->with('success', __('sponsor.deleted_selected'));
}
