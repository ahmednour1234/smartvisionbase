...
{
    // خريطة أسماء الـ Sections كما تُخزن في قاعدة البيانات (عمود section)
    $sections = [
        'zone1' => 'Zone 1 – Affiliates, IBs & Influencers',
        'zone2' => 'Zone 2 – Brokers & Pro Traders',
        'zone3' => 'Zone 3 – Fintech, Services & Public Access',
    ];
    ...
    $categoriesBySection = [
        $sections['zone1'] => [
            ...
            'Affiliate Network Representative',
            'Performance Marketer',
            'Referral Partner',
            'Social Media Influencer',
            'Financial YouTuber / Blogger',
        ...
        $sections['zone3'] => [
            ...
            'Fintech Company',
            'CRM / Automation Tools Provider',
            'Payment / PSP Solutions',
            'Trading Platform / Tech Provider',
            'Business Consultant',
            'General Visitor (Interested in Forex & Financial Markets)',
    ...
    // نصفي بالاسم/الهاتف/التاريخ/الفورم
    ...
    // تصفية حسب الـ Section (عمود section يحتفظ بالنص الكامل)
    if ($request->filled('section')) {
        $sectionKey = $request->get('section'); // zone1|zone2|zone3
        if (isset($sections[$sectionKey])) {
            $query->where('section', $sections[$sectionKey]);
        }
    }

    // تصفية حسب الـ Category
    if ($request->filled('category')) {
        $query->where('category', $request->category);
    }

    // النوع 1 فقط
    $clients = $query->where('type', 1)->latest()->paginate(10)->withQueryString();

    // الفئات المتاحة حسب الـ section المختار (أو الكل)
    $selectedSectionKey = $request->get('section');
    $availableCategories = [];

    if ($selectedSectionKey && isset($sections[$selectedSectionKey])) {
        $sectionName = $sections[$selectedSectionKey];
        $availableCategories = $categoriesBySection[$sectionName] ?? [];
    } else {
        // دمج كل الفئات لكل الأقسام
        foreach ($categoriesBySection as $cats) {
