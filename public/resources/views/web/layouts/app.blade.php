...
<head>
  ...
  {{-- ====== Favicon ====== --}}
  <link rel="icon" href="{{ asset('public/1754488784-LOGO AFFILIATE-06 1.png') }}" type="image/x-icon">
  ...
  {{-- ====== Project CSS (preload → stylesheet) ====== --}}
  <link rel="preload" href="{{ ver('public/web/assets/css/bootstrap.css') }}" as="style" onload="this.onload=null;this.rel='stylesheet'">
  <link rel="preload" href="{{ ver('public/web/assets/css/fonts.css') }}"    as="style" onload="this.onload=null;this.rel='stylesheet'">
  <link rel="preload" href="{{ ver('public/web/assets/css/style.css') }}"    as="style" onload="this.onload=null;this.rel='stylesheet'">
  <noscript>
    <link rel="stylesheet" href="{{ ver('public/web/assets/css/bootstrap.css') }}">
    <link rel="stylesheet" href="{{ ver('public/web/assets/css/fonts.css') }}">
    <link rel="stylesheet" href="{{ ver('public/web/assets/css/style.css') }}">
  </noscript>

  {{-- ====== Bootstrap Icons: لازم يتحمّل فعليًا عشان تظهر أيقونة واتساب ====== --}}
  ...
  {{-- ====== Pixels from DB (lazy after load) ====== --}}
  @php $pixels = \App\Models\Pixel::where('active', true)->get(); @endphp
  <script>
    window.__appPixels = @json($pixels->map(fn($p)=>['name'=>$p->name,'id'=>$p->pixel_id]));
...
<body>
    ...
    <div class="countdown-section" id="countdownSection">
      <div class="container countdown-container">
        ...
        <div class="action-buttons">
          <a href="{{ route('web.register') }}" class="custom-button-white">{{ __('register_now') }}</a>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      (function(){
        ...
        function tick(){
          ...
          if (diff <= 0) { el.textContent = "{{ __('event_started') }}"; return; }
          const d = Math.floor(diff/86400000);
          const h = Math.floor((diff%86400000)/3600000);
          const m = Math.floor((diff%3600000)/60000);
          const s = Math.floor((diff%60000)/1000);
          el.innerHTML = `<span>${d} {{ __('days') }}</span><span>${h} {{ __('hours') }}</span><span>${m} {{ __('minutes') }}</span><span>${s} {{ __('seconds') }}</span>`;
        }
  ...
  {{-- Project JS (defer) --}}
  <script defer src="{{ ver('public/web/assets/js/core.min.js') }}"></script>
  <script defer src="{{ ver('public/web/assets/js/script.js') }}"></script>
</body>
</html>
