        ...
        <div class="row">
            <div class="col-xl-4 col-lg-5">
                <div class="team-details__left">
                    ...
                    <div class="mt-4">
                        <h3 class="stat-number">{{ $company->name_en }}</h3>
                        @if($company->category)
                            <p class="stat-number">{{ $company->category }}</p>
                        @endif
                      @if($company->number_of_followers)
  <span class="followers-bar" style="display:inline-flex;align-items:center;gap:6px;">
    {{-- Instagram outline 12px --}}
    ...
    <p class="stat-number" style="margin:0; font-size:20px;">{{ $company->number_of_followers }}</p>
  </span>
@endif
         @if($company->followers_ticktock)
  <span class="followers-bar" style="display:inline-flex;align-items:center;gap:6px;">
    {{-- Instagram outline 12px --}}
    <svg width="22" height="22" viewBox="0 0 24 24" aria-hidden="true" focusable="false" style="display:block">
    ...
    <p class="stat-number" style="margin:0; font-size:20px;">{{ $company->followers_ticktock }}</p>
  </span>
@endif
                    </div>
...
<div id="votePopup" class="vote-popup-overlay" aria-hidden="true">
    <div class="vote-popup" role="dialog" aria-modal="true" aria-labelledby="voteTitle" aria-describedby="voteMessage">
        ...
        <div class="vote-head">
            ...
            <div class="vote-title" id="voteTitle">{{ __('Voting Result') }}</div>
        </div>

        <div id="voteMessage" class="vote-message">…</div>

        <div class="vote-actions">
            <button class="btn-ghost" type="button" onclick="closeVotePopup()">{{ __('Close') }}</button>
            <a href="{{ url()->previous() }}" class="btn-primary-gradient">{{ __('Back') }}</a>
        </div>
...
async function submitVote(companyId) {
    ...
    btn.textContent = '{{ __("Voting...") }}';

    try {
        const res = await fetch(`{{ url('/vote') }}/${companyId}`, {
            method: 'POST',
            headers: {
        ...
        if (res.ok) {
            ...
            titleEl.textContent = `{{ __('Thanks! Your vote was recorded') }}`;
            msgBox.textContent = (data && data.message) ? data.message : `{{ __('Your vote has been counted successfully.') }}`;
            btn.textContent = '{{ __("Voted") }}';
            throwConfetti();
        } else {
            ...
            titleEl.textContent = `{{ __('Oops! Vote failed') }}`;
            msgBox.textContent = (data && data.message) ? data.message : `{{ __('Something went wrong while submitting your vote.') }}`;
        ...
        iconBox.innerHTML = `<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 2a10 10 0 1 0 0 20A10 10 0 0 0 12 2Zm1 14h-2v-2h2v2Zm0-4h-2V7h2v5Z"/></svg>`;
        document.getElementById('voteTitle').textContent = `{{ __('Network Error') }}`;
        document.getElementById('voteMessage').textContent = `{{ __('Please check your connection and try again.') }}`;
        btn.textContent = originalLabel;
        btn.disabled = false;
        openVotePopup();
    } finally {
