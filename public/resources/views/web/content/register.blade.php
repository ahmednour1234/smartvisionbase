...
@php
  ...
  // Build options list [ISO, "Country (+code)"]
  $options = [];
  foreach ($orderedIso as $iso) {
      if (isset($dialMap[$iso]) && isset($countryNames[$iso])) {
          $options[] = [$iso, $countryNames[$iso] . ' (' . $dialMap[$iso] . ')'];
      }
  }

  // Selected values from previous step (if any)
...
.form-icon{position:absolute;top:50%;left:18px;transform:translateY(-50%);font-size:20px;color:#E73701;margin-right:12px}
.btn-square{width:100%;padding:16px;background:linear-gradient(90deg,#E73701,#000);border:none;border-radius:50px;color:#fff;font-size:18px;font-weight:700;cursor:pointer;transition:transform .2s ease,opacity .3s ease}
...
<section>
  <div class="container">
    <div class="form-card">
      ...
      <form action="{{ route('web.register.store') }}" method="post" id="reg-form">
        @csrf

        @if(!empty($selectedSection))  <input type="hidden" name="section"  value="{{ $selectedSection }}"> @endif
        @if(!empty($selectedCategory)) <input type="hidden" name="category" value="{{ $selectedCategory }}"> @endif
        ...
        <div class="form-wrap">
          ...
          <div class="phone-flex">
            <select class="form-select" name="country_code" id="country_code" required>
              @foreach($options as [$iso, $label])
                @php
                  $dial = $dialMap[$iso];
                  $selected = ($iso === $defaultIso) || ($dial === $defaultDial);
                @endphp
...
{{-- libphonenumber for accurate validation --}}
...
document.getElementById('reg-form').addEventListener('submit', function (e) {
  ...
  const rawOriginal = phoneEl.value.trim();

  // ------------------- FIX (NO UI CHANGE): normalize BEFORE validate -------------------
  // Goal:
  // - If user typed full international (+971.... or 00971....), convert to national number
  // - And set the dropdown to detected country if possible
  // - So backend will never receive: country_code + phone_with_code
  try {
    const lib = (window.libphonenumber || libphonenumber);

    // If user entered something that looks international, parse it
    const maybeInt = rawOriginal.replace(/\s+/g,'');
    const looksInternational = maybeInt.startsWith('+') || maybeInt.startsWith('00');

    if (looksInternational) {
      const parsedInt = lib.parsePhoneNumberFromString(maybeInt, iso);
      if (parsedInt && parsedInt.isValid()) {
        // set select by detected country
        const detectedIso = (parsedInt.country || '').toUpperCase();
        if (detectedIso) {
          const opt = [...select.options].find(o => (o.dataset?.iso || '').toUpperCase() === detectedIso);
          if (opt) select.value = opt.value;
        }
        // set phone input to NATIONAL NUMBER only
        phoneEl.value = String(parsedInt.nationalNumber || '').trim();
      }
    } else {
      // If user typed local number but with leading + or contains selected dial, clean it
      const digits = rawOriginal.replace(/\D+/g,'');           // digits only
      const dialDigits = String(dial).replace(/\D+/g,'');      // digits only
      // If digits start with dialDigits, strip it
      if (dialDigits && digits.startsWith(dialDigits)) {
        phoneEl.value = digits.slice(dialDigits.length).replace(/^0+/, '');
      }
    }
  } catch (_) {
    // ignore, continue normal validation below
  }
  // ------------------- END FIX -------------------
  ...
  try {
    ...
    const parsed = (window.libphonenumber || libphonenumber).parsePhoneNumberFromString(candidate, iso);
    if (parsed && parsed.isValid() && (!parsed.country || parsed.country === iso)) {
      valid = true;
    }
  } catch (_) {}

  // Fallback regex (practical)
  if (!valid) {
    const fb = {
...
// ======================= Continuous auto-detect for dial code (no external API) =======================
// Keeps updating until the user starts typing a phone number, then stops.
...
document.getElementById('reg-form').addEventListener('submit', stopAutoDetect);

startAutoDetect();

function startAutoDetect(){
  runDetectionOnce(); // quick try
  ...
  intervalId = setInterval(() => { if (autoActive) runDetectionOnce(); }, 8000);
}
...
function updateDialFromCoords(lat, lon){
  // Special priority for Egypt (common case)
