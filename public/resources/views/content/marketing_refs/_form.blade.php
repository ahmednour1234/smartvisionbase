{{-- resources/views/dashboard/marketing_refs/_form.blade.php --}}
@php
  ...
  <div class="mr-card">
    ...
    <div class="mr-head">
      ...
      <div class="mr-switch">
        ...
        <input id="mrActive" type="checkbox" name="active" value="1" {{ old('active', $ref->active ?? true) ? 'checked' : '' }}>
        <label for="mrActive" id="activeLabel">{{ old('active', $ref->active ?? true) ? 'نشط' : 'متوقف' }}</label>
      </div>
    </div>
        ...
        <div class="mr-grid">
          <div class="mr-field">
            ...
            <input type="text" name="name" value="{{ old('name', $ref->name ?? '') }}"
                   class="mr-input @error('name') is-invalid @enderror" required>
            @error('name') <div class="mr-error">{{ $message }}</div> @enderror
          </div>
          ...
          <div class="mr-field">
            ...
            <input type="text" name="code" value="{{ old('code', $ref->code ?? '') }}"
                   class="mr-input mr-mono @error('code') is-invalid @enderror" placeholder="اتركه فارغًا لتوليد تلقائي">
            @error('code') <div class="mr-error">{{ $message }}</div> @enderror
          ...
          <div class="mr-field">
            <div class="mr-inline">
              ...
              <label class="mr-label" style="font-weight:500;">
                <input type="checkbox" id="unlimited-uses" {{ old('max_uses', $ref->max_uses ?? null) ? '' : 'checked' }}>
                <span style="margin-right:6px;">بدون حد أقصى</span>
              </label>
            </div>
            <input type="number" min="1" name="max_uses" id="max_uses_input"
                   value="{{ old('max_uses', $ref->max_uses ?? '') }}"
                   class="mr-input @error('max_uses') is-invalid @enderror"
                   placeholder="مثال: 100" {{ old('max_uses', $ref->max_uses ?? null) ? '' : 'disabled' }}>
            @error('max_uses') <div class="mr-error">{{ $message }}</div> @enderror
          </div>

          <div class="mr-field">
            <label class="mr-label">تاريخ الانتهاء (اختياري)</label>
            <input type="datetime-local" name="expires_at" value="{{ $expiresValue }}"
                   class="mr-input @error('expires_at') is-invalid @enderror">
            @error('expires_at') <div class="mr-error">{{ $message }}</div> @enderror
          </div>
        </div>
      </div>

      {{-- Section: المسارات المسموح بها --}}
      <div class="mr-sec">
        ...
        <div id="paths-wrapper" class="mr-paths" style="margin-top:10px;">
          @php $paths = count($oldAllowed) ? $oldAllowed : ['']; @endphp
          @foreach($paths as $i => $p)
            <div class="mr-path-row path-row">
              <input type="text" name="allowed_paths[]" value="{{ $p }}"
                     class="mr-input" placeholder="/register">
              <button type="button" class="mr-btn mr-btn--muted remove-path" title="حذف">حذف</button>
            </div>
          @endforeach
        </div>
        @error('allowed_paths') <div class="mr-error">{{ $message }}</div> @enderror
      </div>

      {{-- Section: رابط المشاركة (slug) --}}
      @if(isset($ref))
        <div class="mr-sec">
          <div class="mr-sec-title">رابط المشاركة</div>
          <div class="mr-share">
            {{-- الرابط الأساسي --}}
            @php $baseShare = route('ref.hit', $ref); @endphp
            <div class="mr-share-row">
              <div class="mr-url mr-mono" id="mrBaseUrl">{{ $baseShare }}</div>
              <button type="button" class="mr-btn mr-btn--brand copy-share" data-copy="{{ $baseShare }}">نسخ</button>
            </div>

            {{-- رابط لمسار محدد (اختياري) --}}
            <div class="mr-share-row">
              <select id="mrPathSelect" class="mr-select" style="max-width:260px;">
                @foreach($previewPaths as $opt)
                  <option value="{{ $opt }}">{{ $opt }}</option>
                @endforeach
              </select>
              <div class="mr-url mr-mono" id="mrPathUrl">{{ rtrim($baseShare,'/') . '/' . ltrim($previewPaths[0] ?? '/register', '/') }}</div>
    ...
    <div class="mr-foot">
      ...
      <a href="{{ route('marketing-refs.index') }}" class="mr-btn mr-btn--muted">إلغاء</a>
    </div>
  </div>
</div>
