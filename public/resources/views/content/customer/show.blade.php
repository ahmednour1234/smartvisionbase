{{-- resources/views/content/customer/show.blade.php --}}
  ...
  <div class="page-hero mb-3">
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
      <div class="d-flex align-items-center gap-3">
        <a href="{{ route('dashboard.leads.index') }}" class="back-link btn-u btn-soft" style="padding-inline:12px">
      ...
      <div class="d-flex align-items-center gap-2">
        <span class="lead-chip {{ $chipClass }}">
          <i class="fa fa-tag ic fa-fw"></i> {{ ucfirst(str_replace('_',' ',$lead->status)) }}
        </span>
        ...
        @if(!$locked)
          ...
          <form action="{{ route('dashboard.leads.accept',$lead) }}" method="post" class="ms-1">
            @csrf
        ...
        @else
          ...
          @if($lead->status === 'accept')
            @if($qrImgUrl)
              <a href="{{ $qrImgUrl }}" download class="btn-u btn-dark">
        ...
        <div class="col-6 col-md-3">
          <div class="card-white">
            <div class="card-body">
              ...
              <div class="v fs-5">{{ $lead->comments->count() }}</div>
            </div>
          </div>
        </div>
        <div class="col-6 col-md-3">
          <div class="card-white">
            <div class="card-body">
              ...
              <div class="v fs-5">{{ $lead->callLogs->count() }}</div>
            </div>
          </div>
        </div>
        <div class="col-6 col-md-3">
          <div class="card-white">
            <div class="card-body">
              ...
              <div class="v">
                @if(isset($lastActionType) && $lastActionType)
                  {{ $lastActionType === 'comment' ? 'Comment' : 'Call' }}
                  <span class="note">• {{ \Carbon\Carbon::parse($lastActionAt)->diffForHumans() }}</span>
                @else
                  —
                @endif
              </div>
        ...
        <div class="col-6 col-md-3">
          ...
          <div class="card-white">
            <div class="card-body">
              ...
              <div class="v {{ $overdue ? 'danger-text' : '' }}">
      ...
      <ul class="nav nav-pills mb-2" id="leadTabs" role="tablist">
        <li class="nav-item" role="presentation">
          <button class="nav-link active" id="comments-tab" data-bs-toggle="pill" data-bs-target="#comments-pane" type="button" role="tab">
            <i class="fa fa-comments ic fa-fw"></i> Comments <span class="badge bg-secondary ms-1">{{ $lead->comments->count() }}</span>
          </button>
        </li>
        <li class="nav-item" role="presentation">
          <button class="nav-link" id="calls-tab" data-bs-toggle="pill" data-bs-target="#calls-pane" type="button" role="tab">
            <i class="fa fa-phone ic fa-fw"></i> Calls <span class="badge bg-secondary ms-1">{{ $lead->callLogs->count() }}</span>
          </button>
        </li>
      </ul>
        ...
        <div class="tab-pane fade show active" id="comments-pane" role="tabpanel" aria-labelledby="comments-tab">
          ...
          <div class="card-white">
            <div class="card-head d-flex align-items-center justify-content-between">
              ...
              <div class="note">{{ $lead->comments->count() }} items</div>
            </div>
            <div class="card-body">
              <div class="timeline">
                @forelse($lead->comments as $c)
                  <div class="tl-item">
                    <div class="d-flex justify-content-between">
                      <div class="fw-semibold">{{ $c->user?->name ?? 'Unknown' }}</div>
                      <div class="note">{{ \Carbon\Carbon::parse($c->created_at)->format('Y-m-d H:i') }}</div>
                    </div>
                    <div class="mt-1">{{ $c->comment }}</div>
                    <form action="{{ route('dashboard.leads.comments.destroy',$c) }}" method="post" class="mt-2">
                      @csrf @method('DELETE')
        ...
        <div class="tab-pane fade" id="calls-pane" role="tabpanel" aria-labelledby="calls-tab">
            ...
            <div class="card-body">
              <form method="post" action="{{ route('dashboard.leads.calllogs.store',$lead) }}">
                ...
                <div class="mt-3 d-flex gap-2 justify-content-end">
                  <a class="btn-u btn-soft" href="tel:{{ $lead->phone }}"><i class="fa fa-phone ic fa-fw"></i> Call Now</a>
                  <a class="btn-u btn-soft" href="mailto:{{ $lead->email }}"><i class="fa fa-envelope ic fa-fw"></i> Send Email</a>
          ...
          <div class="card-white">
            <div class="card-head d-flex align-items-center justify-content-between">
              ...
              <div class="note">{{ $lead->callLogs->count() }} items</div>
            </div>
            <div class="card-body">
              <div class="timeline">
                @forelse($lead->callLogs as $l)
                  <div class="tl-item">
                    <div class="d-flex justify-content-between">
                      <div class="fw-semibold">{{ $l->user?->name ?? 'Unknown' }}</div>
                      <div class="note">{{ \Carbon\Carbon::parse($l->called_at)->format('Y-m-d H:i') }}</div>
                    </div>
                    <div class="note mt-1">
                      <i class="fa fa-stopwatch ic fa-fw me-1"></i>{{ $l->duration_sec }}s
                      <span class="mx-2">•</span>
                      <i class="fa fa-wave-square ic fa-fw me-1"></i>{{ $l->outcome }}
                    </div>
                    @if($l->notes)
                      <div class="mt-1">{{ $l->notes }}</div>
                    @endif
                  </div>
                @empty
      ...
      <div class="sticky-sidebar">
        ...
        <div class="card-white mb-3">
          ...
          <div class="card-body text-center">
            ...
            @else
              <img src="{{ $qrImgUrl }}" alt="QR" class="qr-img img-fluid mb-2">
              <div class="d-flex justify-content-center gap-2">
                <a href="{{ $qrImgUrl }}" target="_blank" class="btn-u btn-soft">
        ...
        <div class="card-white mb-3">
          ...
          <div class="card-body">
            <div class="kv mb-2"><div class="k">Owner</div><div class="v">{{ $lead->assignedUser?->name ?? 'Unassigned' }}</div></div>
            <div class="kv mb-2"><div class="k">Source</div><div class="v">{{ $lead->source ?: '—' }}</div></div>
            <div class="kv mb-2"><div class="k">Job</div><div class="v">{{ $lead->job ?: '—' }}</div></div>
            <div class="kv mb-2"><div class="k">Created at</div><div class="v">{{ $lead->created_at?->format('Y-m-d H:i') }}</div></div>
            <div class="kv"><div class="k">Last update</div><div class="v">{{ $lead->updated_at?->diffForHumans() }}</div></div>
          </div>
        </div>
          ...
          <div class="card-body">
            <form method="post" action="{{ route('dashboard.leads.update',$lead) }}">
              ...
              <input type="datetime-local" class="form-control mb-2"
                     name="next_call_at"
                     value="{{ $lead->next_call_at ? \Carbon\Carbon::parse($lead->next_call_at)->format('Y-m-d\TH:i') : '' }}">
              {{-- Hidden fillables to keep values --}}
              <input type="hidden" name="name" value="{{ $lead->name }}">
              <input type="hidden" name="email" value="{{ $lead->email }}">
              <input type="hidden" name="phone" value="{{ $lead->phone }}">
              <input type="hidden" name="job" value="{{ $lead->job }}">
            ...
            @php $overdue = $lead->next_call_at && \Carbon\Carbon::parse($lead->next_call_at)->lt(now()) && in_array($lead->status,['new','follow_up']); @endphp
            @if($overdue)
              <div class="danger-text mt-2"><i class="fa fa-triangle-exclamation ic fa-fw me-1"></i> Overdue follow-up</div>
            @elseif($lead->next_call_at)
              <div class="ok-text mt-2"><i class="fa fa-bell ic fa-fw me-1"></i> {{ \Carbon\Carbon::parse($lead->next_call_at)->diffForHumans() }}</div>
            @endif
          </div>
        </div>
        ...
        <div class="card-white">
          ...
          <div class="card-body">
            <div class="d-flex flex-wrap gap-2">
              @if(!$locked)
                @foreach (['new','follow_up','accept','reject','lost'] as $st)
                  <form action="{{ route('dashboard.leads.status',$lead) }}" method="post">
                    @csrf
                    <input type="hidden" name="status" value="{{ $st }}">
                    <button class="btn-u btn-soft">{{ ucfirst(str_replace('_',' ',$st)) }}</button>
                  </form>
                @endforeach
              @else
              ...
              @endif
              <a class="btn-icon" href="tel:{{ $lead->phone }}" title="Call"><i class="fa fa-phone ic fa-fw"></i></a>
              <a class="btn-icon" href="mailto:{{ $lead->email }}" title="Email"><i class="fa fa-envelope ic fa-fw"></i></a>
            </div>
          </div>
        </div>
      </div> {{-- /sticky --}}
...
<div class="modal fade" id="statusLockedModal" tabindex="-1" aria-labelledby="statusLockedLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content card-white">
      ...
      <div class="card-body">
        <p class="mb-3">
          This lead’s status is <strong>{{ ucfirst(str_replace('_',' ',$lead->status)) }}</strong>, therefore
